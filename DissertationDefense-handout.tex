...
State-space models \pause
\begin{itemize}
\item Dynamic linear models \pause
\item Sequential estimation
\end{itemize} \pause
Sequential Monte Carlo methods \pause
\begin{itemize}
\item particle filters \pause
...
\begin{itemize}
\item dependence of observations on the unobserved state \pause
\end{itemize}
\end{enumerate}
May contain unknown fixed parameters
}

\frame{\frametitle{State-space models}
Notation \pause
\begin{itemize}
\item $x_t$ - unobserved state vector at time $t$, $t=0,1,2,\ldots$ \pause
\item $y_t$ - observed data vector at time $t$, $t=1,2,\ldots$ \pause
...
\item $p(y_t|x_t,\theta)$ - likelihood (observation equation) \pause
\item $p(x_0,\theta)$ - prior distribution on initial state and fixed parameters \pause
\item $(y_t \perp y_{1:t-1},y_{t+1:T}) | x_t,\theta$ and $(x_t \perp x_{0:t-2},y_{1:t-1}) | x_{t-1},\theta$
\end{itemize}
\begin{figure}[ht]
\centering
...
\begin{align*}
\mbox{obs. eqn.:  } & y_t = U_t\beta + F_tx_t + v_t, \quad & v_t &\stackrel{iid}{\sim} \mbox{N}(0,V) \\
\mbox{state eqn.:  } & x_t = Gx_{t-1} + w_t, \quad & w_t &\stackrel{iid}{\sim} \mbox{N}(0,W) \\
\mbox{prior:  } & x_0 \sim \mbox{N}(m_0, C_0) \quad & &
\end{align*} \pause
\begin{align*}
y_t:& \ q \times 1 & U_t:& \ q \times d & \beta:& \ d \times 1 & & \\
F_t:& \ q \times p & x_t:& \ p \times 1 & v_t:& \ q \times 1 & V:& \ q \times q \\
G:& \ p \times p & W:& \ p \times p & m_0:& \ p \times 1 & C_0:& \ p \times p
\end{align*} \pause
\begin{itemize}
...
%\begin{align*}
%y_t &= U_t\beta + w_t \\
%w_t &= \phi_1w_{t-1} + \cdots + \phi_Pw_{t-P} + \epsilon_t + \gamma_1\epsilon_{t-1} + \cdots + \gamma_Q\epsilon_{t-Q} \\
%t &\ge \mbox{max}(P,Q+1) \\
%\epsilon_j &\stackrel{iid}{\sim} \mbox{N}(0,\sigma^2), \ j \ge 1
%\end{align*} \pause
%\[
%\begin{array}{ll}
%x_t &= (x_{t,1} ,x_{t,2}, \ldots, x_{t,m})' \\ \pause
...
%Invertibility: \pause roots of $\gamma(z)$ lie outside of unit circle, where
...
\end{align*} \pause
Let $x_t$ be univariate, follow an AR(1) process, i.e. $W = \sigma^2_s$ and $G = \phi$ \pause \\
Let $y_t$ be univariate, $V = \sigma^2_m$, $U_t = (1,u_t)$, and $\beta = (\beta_0,\beta_1)'$ \pause \\
%Let $p(x_0) = \delta_{0}(x_0)$, i.e. $x_0 = 0$ \pause \\
Dynamic intercept model, i.e. reg. with AR(1)+WN errors: $F_t = 1 \ \forall t$ \pause
\begin{align*}
y_t &= (\beta_0 + x_t) + \beta_1u_t + v_t, \ v_t \stackrel{iid}{\sim}\mbox{N}(0,\sigma^2_m) \\
y_t &= \beta_0 + \beta_1u_t + x_t + v_t
\end{align*} \pause
Dynamic slope model: $F_t = u_t$ \pause \\
\[y_t = \beta_0 + (\beta_1 + x_t)u_t + v_t, \ v_t \stackrel{iid}{\sim}\mbox{N}(0,\sigma^2_m)\] \pause
Unknown fixed parameters $\theta = (\beta_0,\beta_1,\phi,\sigma^2_s,\sigma^2_m)'$ \\ \pause
Constrain initial state: $p(x_0) = \delta_{0}(x_0)$, i.e. $x_0 = 0$
}

\frame{\frametitle{Dynamic regression models}
Dynamic intercept and slope model: \pause
\begin{align*}
F_t &= (1, u_t) & G &= \left(\begin{array}{cc} \phi & 0 \\ 0 & \rho \end{array}\right) & W &= \left(\begin{array}{cc} \sigma^2_s & 0 \\ 0 & \sigma^2_b \end{array}\right) \\
...
Constrain initial state: $p(x_0) = \delta_{(0,0)}(x_0)$, i.e. $x_{0,1} = x_{0,2} = 0$
}

\frame{\frametitle{Dynamic regression models}
Model notation: $M_{ijk}$ \pause
\begin{itemize}
\item $i$ is 1 or 0 indicating whether intercept follows AR(1)
\item $j$ is 1 or 0 indicating whether slope follows AR(1)
...
\end{itemize}
Analytical tractability only in special cases \pause
\begin{itemize}
...
Based on a sequential use of importance sampling \\ \pause
Approximates $p(x_t,\theta|y_{1:t})$ through a weighted Monte Carlo realization in terms of $J$ particles\pause, i.e.
...
\item $w_t^{(j)}$ - weight of particle $j$ with $\sum_{j=1}^J w_t^{(j)}=1$ 
%\item $\delta_{w_0}(w) = \left\{\begin{array}{ll} 1, & \ \ w = w_0 \\ 0, & \ \ w \ne w_0 \end{array}\right.$ \pause
\end{itemize}
}

\frame{\frametitle{Bootstrap filter (BF) \citep{Gord:Salm:Smit:nove:1993,Kita:mont:1996}} \pause
Assume $\theta$ known, approximate $p(x_t|y_{1:t})$ \\ \pause
Initial weights $w_0^{(j)}$ and locations $x_0^{(j)}$ for all $j$ \\ \pause
Sample from $p(x_0)$ with uniform weights \\ \pause
Given a particle approximation to $p(x_t|y_{1:t})$, approximate $p(x_{t+1}|y_{1:t+1})$ \\ \pause
For each particle $j=1,\ldots,J$: \pause
\begin{enumerate}
\item Propagate: sample $x_{t+1}^{(j)} \sim p\left( x_{t+1}\left|x_t^{(j)}\right.\right)$ \pause
\item Update weights: $\tilde{w}_{t+1}^{(j)} = p\left(y_{t+1}\left|x_{t+1}^{(j)}\right.\right)$ \pause
\end{enumerate}
Renormalize weights: $w_{t+1}^{(j)} = \tilde{w}_{t+1}^{(j)}\left/ \sum_{l=1}^J \tilde{w}_{t+1}^{(l)} \right.$ \\ \pause
Optional: resample the particles to balance weights \\ \pause
%\item Resample: (optionally) sample an index $k\in\{1,\ldots,j,\ldots,J\}$ with associated probabilities $\left\{w_t^{(1)},\ldots,w_t^{(j)},\ldots,w_t^{(J)}\right\}$, \pause
%\end{enumerate}
Apply recursively
}

\frame{\frametitle{Auxiliary particle filter (APF) \citep{Pitt:Shep:filt:1999}} \pause
Problem: Degeneracy - $w_t^{(j)}$ small where $p\left(y_{t}\left|x_{t}^{(j)}\right.\right)$ is small \\ \pause
...
\item For each particle $j=1,\ldots,J$,
	...
	\item Calculate weights: $\tilde{w}_{t+1}^{(j)} = p\left(y_{t+1}\left|x_{t+1}^{(j)}\right.\right) \left/ p\left(y_{t+1}\left|\mu_{t+1}^{(k)}\right.\right)\right.$
...
V_t &= \mbox{ sample covariance of } \theta_t^{(1)},\ldots,\theta_t^{(J)} \\
a^2 &= 1 - h^2 \\
...
%    \item Renormalize: $w_{t+1}^{(j)} = \tilde{w}_{t+1}^{(j)}\left/\sum_{l=1}^J \tilde{w}_{t+1}^{(l)}. \right.$
%	\end{enumerate}
%\end{enumerate}
%}
...
\item $q\left(x_{0:t},\theta|x_{0:t}^{(j)},\theta_t^{(j)}\right)$ satisfies $\int_{\theta}\int_{x_{0:t}} p(x_{0:t},\theta|y_{1:t})q\left(x_{0:t},\theta|x_{0:t}^{(j)},\theta_t^{(j)}\right)\mbox{d}x_{0:t}\mbox{d}\theta = p(x_{0:t},\theta|y_{1:t})$ \\ \pause
\item $\left(x_{0:t}^{(j)},\theta_t^{(j)}\right) \stackrel{.}{\sim} p(x_{0:t},\theta|y_{1:t}) \Rightarrow \theta^* \stackrel{.}{\sim} p(\theta|y_{1:t})$
\end{itemize}
}

%\frame{\frametitle{Resample-move (RM) \citep{Gilk:Berz:foll:2001}}
%Given a particle approximation to $p(x_{0:t},\theta|y_{1:t})$, approximate $p(x_{0:t+1},\theta|y_{1:t+1})$ \\ \pause
...
& & &p(y_{t+1}|x_t,\theta)p(x_t,s_t,\theta|y_{1:t})\mbox{d}x_t\mbox{d}s_t
\end{align*} \pause
\begin{itemize}
\item looks ahead, like in APF, through $p(y_{t+1}|x_t,\theta)$ \pause
\item propagate states through $p(x_{t+1}|y_{t+1},x_t,\theta)$ \pause
\item track $s_{t+1}$ through $S(y_{t+1},x_{t+1},s_t)$ \pause
\item need $p(y_{t+1}|x_t,\theta)$, $p(x_{t+1}|y_{t+1},x_t,\theta)$, and $p(\theta|s_t)$
\end{itemize}
}

%\frame{\frametitle{Particle learning (PL) \citep{Carv:Joha:Lope:Pols:part}}
%Add $s_t$ to particles: $\left(x^{(j)}_t,\theta^{(j)}_t,s_t^{(j)}\right)$ \\ \pause
%Given a particle approximation to $p(x_t,s_t,\theta|y_{1:t})$, approximate $p(x_{t+1},s_{t+1},\theta|y_{1:t+1})$ \\ \pause
%For each particle $j=1,\ldots,J$:
%\begin{enumerate}
...
Can only use PL in special cases: \pause
\begin{itemize}
\item first-order DLM with common state and observation variance \pause
\item dynamic regression \pause
\item conjugate priors on $\theta$ \pause
\end{itemize}
...
\frame{\frametitle{Estimating the marginal likelihood} \pause
Consider data $y_1,\ldots,y_T$ \\ \pause
Calculate $p(y_{1:T})$ through $p(y_1)$ and $p(y_t|y_{1:t-1})$ for $t=2,\ldots,T$ \\
...
Example: first-order DLM with $\theta = \lambda = 1$
\begin{itemize}
\item single simulated time series of length $T = 100$ \pause
\item consider three models: $M_1: \lambda = 0.5, \ M_2: \lambda = 1, \ M_3: \lambda = 2$ \pause
\item $p(M_i) = 1/3$ for all $i$ \pause
\item can calculate true $p(y_{1:T})$, $p(M_i|y_{1:T})$ \pause
\item estimate $p(M_i|y_{1:T})$ using KDPF, RM, and PL \pause
\item 20 separate runs on each model with each algorithm \pause
\item $J = 100, 500, 1000, 5000$ particles
\end{itemize}
}
...
\frame{\frametitle{Canonical hrf}
\centering
...
\item $5 \times 5 \times 5$ voxel regions from 6 brain areas \pause
\item preprocessing
\end{itemize}
}
...
\item $\sigma^2$ - measurement variance
\end{itemize}
}
...
\end{align*} \pause
\begin{itemize}
\item ordinary least squares (OLS) estimation, t-test with $T - 2$ df \pause
...
%\epsilon_j &\stackrel{iid}{\sim} \mbox{N}(0,\sigma^2), \ j \ge 0 \\
%\theta &= (\beta_0,\beta_1,\phi_1,\ldots,\phi_P,\gamma_1,\ldots,\gamma_Q,\sigma^2)'
%\end{align*} \pause
%\begin{itemize}
...
\item adjust degrees of freedom in t-test (REMLc)
\end{itemize}
}

%\frame{\frametitle{Comparing autocorrelation estimation algorithms} \pause
%$M_{100}$: assume regression with AR(1) errors \\
%\begin{align*}
%y_t &= \beta_0 + \beta_1\mbox{conv}_t + x_t \\
%x_t &= \phi x_{t-1} + \epsilon_t \\
%\epsilon_j &\stackrel{iid}{\sim} \mbox{N}(0,\sigma^2), \ j \ge 0 \\
%\theta &= (\beta_0,\beta_1,\phi,\sigma^2)'
%\end{align*} \pause
%Simulate 1000 time series with $\beta_0 = 750$, $\beta_1 = 0$, $\sigma^2 = 10$, and each of $\phi \in \{0.25,0.5,0.75,0.95\}$ \\ \pause
%False positive rate of concluding significant activation
%%ROC curves
%}
%
%\frame{\frametitle{False positive rates}
%\begin{table}
%\resizebox{0.75\textwidth}{!}{\begin{minipage}{\textwidth}
%\centering
%\caption*{False positive rates for simulated fMRI data}
...
\begin{align*}
&y_t = \beta_0 + \beta_1\mbox{conv}_t + v_t, \ v_t \stackrel{iid}{\sim} \mbox{N}(0,\sigma^2_m) \\
&\theta = (\beta_0,\beta_1,\sigma^2_m)'
\end{align*} \pause
Estimate $\theta$ via maximum likelihood \\ \pause
Identifiability problems in $M_{111}$
}

\frame{\frametitle{Fitting dynamic regression models}
\centering
\includegraphics[width=1.0\textwidth]{craig_mle-beta-clusters}
}

%\frame{\frametitle{Model comparison - simulated data}
...
%}
%
%\frame{\frametitle{Running PL on $M_{101}$ and $M_{011}$}
%Need to define: $J$, prior on $x_0$, $\theta = (\beta_0,\beta_1,\phi,\sigma^2_s,\sigma^2_m)'$ \\ \pause
%Prior: $p(x_0, \theta) = p(\beta|\sigma^2_m)p(\sigma^2_m)p(\phi|\sigma^2_s)p(\sigma^2_s)\delta_0(x_0)$
%\begin{align*}
...
%\item $E(\sigma^2_m) = \hat{\sigma}^2_m \quad E(\sigma^2_s) = \hat{\sigma}^2_s$
%\item $Var(\sigma^2_m) = Var(\sigma^2_s) = 500$ \pause
%\end{itemize}
%Three PL runs per simulation, 500 particles \\ \pause
%Estimate $\log p(y_{1:T}|M_{101})$, $\log p(y_{1:T}|M_{011})$
%}
%
%\frame{\frametitle{Identifying dynamic slope model}
%\centering
%\includegraphics[width=1.0\textwidth]{fmri_pl_loglik-M101-1-500-phi-496-594-1}
%}
%
%\frame{\frametitle{Identifying dynamic intercept model}
%\centering
...
\end{itemize}
One PL run / voxel (750 total) under both $M_{101}$ and $M_{011}$, 5000 particles \\ \pause
$p(y_{1:T}|M_{001})$ calculated analytically \\ \pause
\[p(M_i|y_{1:T}) = \frac{p(y_{1:T}|M_i)p(M_i)}{\sum_{i=1}^N p(y_{1:T}|M_i)p(M_i)}\] \pause
$p(M_{101}) = p(M_{011}) = p(M_{001}) = 1/3$
}

\frame{\frametitle{Model comparison using PL - word recognition data}
\begin{table}
\centering
...
\frame{\frametitle{Dynamic slopes in SV-left}
\centering
...
\begin{itemize}
\item state sufficient statistics, Rao-Blackwellization \citep{Douc:Gods:Andr:on:2000} \pause
\item combining algorithms \citep{dukic2012tracking} \pause
\item cross model jumps \citep{berz:gilks:rmcross:2001,zhou:joh:smcmodcomp:2013} \pause
\end{itemize}
fMRI
\begin{itemize}
\item dynamic connectivity \citep{ho:ombao:2005:statespace} \pause
\item borrow information spatially, spatio-temporal modeling
\end{itemize}
}
