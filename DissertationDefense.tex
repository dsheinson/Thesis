...
\item Resample (optional): sample an index $k$ from $\{1,\ldots,j,\ldots,J\}$ with associated probabilities $\{w^{(1)}_{t+1},\ldots,w^{(j)}_{t+1},\ldots,w^{(J)}_{t+1}\}$\pause
\item \label{step:move} Move particles (optional): draw a new particle $\left(x^{(j)}_{0:t+1},\theta^{(j)}_{t+1}\right) \sim q\left(x_{0:t+1},\theta|\tilde{x}^{(k)}_{0:t+1},\theta^{(k)}_t\right)$
\end{enumerate}
}
...
$M_{100}$: Assume AR(1) errors \\ \pause
\begin{align*}
y_t &= \beta_0 + \beta_1\mbox{conv}_t + x_t \\
...
\item $Var(\sigma^2_m) = Var(\sigma^2_s) = 500$ \pause
\end{itemize}
One PL run / voxel (750 total), 5000 particles \\ \pause
\[p(M_i|y_{1:T}) = \frac{p(y_{1:T}|M_i)p(M_i)}{\sum_{i=1}^N p(y_{1:T}|M_i)p(M_i)}\] \pause
