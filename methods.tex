...
This chapter proceeds as follows. In Section \ref{sec:mcmc}, we describe MCMC algorithms used for the epidemic model described in Section \ref{sec:epid} and the dynamic regression models described in Section \ref{sec:dlm:arwn}. In Section \ref{sec:filtering}, we describe several particle filtering strategies and how to apply them to the models outlined in Chapter \ref{ch:models}. In Section \ref{sec:resample}, we discuss the resampling aspect of particle filtering algorithms. In Section \ref{sec:comp}, we show how SMC techniques can be used for model comparison. Lastly, in Section \ref{sec:pmcmc}, we discuss a more efficient MCMC method called particle MCMC that we use to analyze simualted data from the epidemic model.

\section{MCMC algorithms \label{sec:mcmc}}

MCMC methods provide sample-based approximations to a distribution through the generation of dependent samples from distributions whose densities can be evaluated. In this section, we outline the MCMC algorithms that we use to analyze simulated data from the state-space model of a disease epidemic described in Section \ref{sec:epid} and the dynamic regression models described in Sections \ref{sec:dlm:M011} and \ref{sec:dlm:M101}. We also describe a particle MCMC (PMCMC) approach that we found to be more efficient when applied to data from the epidemic model. For more comprehensive descriptions of MCMC and PMCMC methods, we refer the reader to \citet{Robe:Case:mont:2004} and \citet{Andr:Douc:Hol:pmcmc:2010}.

\subsection{MCMC applied to epidemic model \label{sec:mcmc:epid}}

Consider the state-space model of an epidemic described in Section \ref{sec:epid}, where $x_t = (s_t,i_t)'$ is the latent disease state, $\theta = (\beta,\gamma,\nu)$ are the unknown fixed parameters, equation \eqref{eqn:epid:state} describes the evolution of $x_t$ given $x_{t-1}$ and $\theta$, and \eqref{eqn:epid:obs} describes the likelihood of new data, $y_t$, given $x_t$ and $\theta$. The prior distribution, $p(x_0,\theta)$, is assumed to be of the form
\begin{equation}
...
\end{equation}
we derive the \emph{full conditional distribution}, i.e. distribution of a random variable conditional on all of the remaining variables, for each of $x_0, x_1, \ldots, x_T$, $\beta$, $\gamma$, and $\nu$ as
\begin{align}
...
\end{align}
where $p(x|\hdots)$ represents the full conditional distribution of the random variable $x$. Note that since all of the unknown fixed parameters show up only in the state equation of the model, their full conditional distributions do not depend on $y_t$.
...
\item \label{step:gibbs} Sample the states, $x^{(j)}_t$ for $t = 0,1,\ldots,T$, from their full conditional distributions. For $t = 1, 2, \ldots, T$,
    \begin{enumerate}
    \item Draw $x^*_t \sim \mbox{N}\left(x^{(j-1)}_t,\tau^2_{x_t}\right)$.
...
To implement a particle learning algorithm for the local level DLM given by equations \eqref{eqn:ll:obs} and \eqref{eqn:ll:state}, we derive the conditional predictive distribution and filtered distribution of the $x_{t+1}$ conditional on $x_t$ and $\theta$, given by
\begin{align}
