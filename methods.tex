...
\subsection{Auxiliary particle filter (APF) \label{sec:apf}}
...
\subsection{Resample-move algorithm (RM)} \label{sec:rm}
...
\begin{enumerate}
\item Propagate: draw $\tilde{x}^{(j)}_{t+1}$ from $p\left(x_{t+1}|x^{(j)}_t,\theta^{(j)}_t\right)$. Incorporate $\tilde{x}^{(j)}_{t+1}$ into particle $j$ and denote the new augmented particle by $\left(\tilde{x}^{(j)}_{0:t+1},\theta^{(j)}_t\right)$, where $\tilde{x}^{(j)}_{0:t+1} = \left(\tilde{x}_0^{(j)},\tilde{x}_1^{(j)},\ldots,\tilde{x}_t^{(j)},\tilde{x}_{t+1}^{(j)}\right)$,
\item Calculate weights and renormalize:
...
\begin{enumerate}
\item \label{step:move:ll} Sample $\theta^{(j)}_{t+1} \sim \mbox{IG}(a_{t+1}, b_{t+1})$, where
\begin{align*}
a_t &= a_0 + 1/2 + t+1 \\
b_t &= b_0 + \frac{1}{2}\left(\sum_{i=1}^{t+1} \left(y_i - \tilde{x}_i^{(k)}\right)^2 + \frac{1}{\lambda}\sum_{i=1}^t \left(\tilde{x}_i^{(k)} - \tilde{x}_{i-1}^{(k)}\right)^2 + \left(\tilde{x}^{(k)}_0\right)^2\right),\mbox{ and}
\end{align*}
\item Sample $x_{0:t+1}^{(j)}$ using the FFBS algorithm detailed in Steps \ref{ffbs:kal} and \ref{ffbs:sample} of the Gibbs sampler from Section \ref{sec:mcmc:dr} with $T = t+1$ and
...
We consider a particle filtering algorithm called particle learning \citep{Carv:Joha:Lope:Pols:part} that can be applied to a particular class of state-space models which includes DLMs. For models within this class, particle learning prescribes a truly sequential algorithm that samples new values for $\theta$ from $p(\theta|y_{1:t})$ using conditional sufficient statistics. Let $s_t$ denote the sufficient statistics for $\theta$ conditional on the states $x_{0:t}$ (unrelated to $s_t$ in the epidemic model from Section \ref{sec:epid}). Then, we incorporate the sampled values of the sufficient statistics, $s_t^{(j)}$, into the particles, i.e. particle $j$ at time $t$ is now represented by $\left(x_t^{(j)},s_t^{(j)},\theta_t^{(j)}\right)$. We move from an approximation to $p(x_t,\theta|y_{1:t})$ to that of $p(x_{t+1},\theta|y_{1:t+1})$ by the following procedure for each particle $j = 1,2,\ldots,J$:
\begin{enumerate}
\item Calculate weights and renormalize:
    \[\tilde{w}_{t+1}^{(j)} = p\left(y_{t+1}\left|x_t^{(j)},\theta_t^{(j)}\right.\right) \qquad w_{t+1}^{(j)} = \tilde{w}_{t+1}^{(j)} \left/ \sum_{l=1}^J \tilde{w}_{t+1}^{(l)}\right.,\]
\item Resample: sample an index $k \in \{1,\ldots,j,\ldots,J\}$ with associated probabilities $\left\{w_{t+1}^{(1)},\ldots,w_{t+1}^{(j)},\ldots,w_{t+1}^{(J)}\right\}$,
\item Propagate: sample $x_{t+1}^{(j)} \sim p\left(x_{t+1}\left|y_{t+1},x_t^{(k)},\theta_t^{(k)}\right.\right)$,
...
\begin{equation}
p(\theta|y_{1:t},x_{0:t}) \propto \left(\prod_{k=1}^t p(y_k|x_k,\beta,\sigma^2_m)p(x_k|x_{k-1},\phi,\sigma^2_s)\right)p(\beta,\sigma^2_m)p(\phi,\sigma^2_s). \label{eqn:pl:dr:theta}
\end{equation}
...
\end{align}
where $\xi_{m_0} = \xi_{s_0} = 0$. We update $\xi_{m_t}$ and $\xi_{s_t}$ in the recursive map and calculate the inverse-gamma rate parameters $b_{m_t}$ and $b_{s_t}$ according to
\begin{align}
...
\end{equation}
Thus, $p(y_{1:t})$ can be calculated through $p(y_1)$ and the one-step ahead predictive densities $p(y_k|y_{1:k-1})$ for $k=2,\ldots,t$, according to
\begin{equation}
...
\begin{align}
\mbox{If } t = 1, & \nonumber \\
p(y_1) &\approx \left\{\begin{array}{ll} \sum_{j=1}^J w^{(j)}_{0}\tilde{w}^{(j)}_1, & \mbox{for the BF, RM, and PL} \\ \left(\frac{1}{J}\sum_{j=1}^J \tilde{w}^{(j)}_{0}\right)\left(\sum_{j=1}^J\tilde{g}^{(j)}_1\right) & \mbox{for the APF and KDPF} \end{array} \right., \ \mbox{and} \label{eqn:onestep:pf:init} \\
 & \nonumber \\
\mbox{if } t \ge 2, & \nonumber \\
