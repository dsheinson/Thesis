...
\end{align}
where $p(w|\hdots)$ represents the full conditional distribution of $w$, for any $w$. Note that since each of the unknown fixed parameters $\beta$, $\gamma$, and $\nu$ is present only in the state equation of the model, their full conditional distributions do not depend on $y_t$. By the same argument $p(y_t|x_t,\theta)$ reduces to $p(y_t|x_t)$.
...
The output of this algorithm is a dependent chain of samples that approximates $p(x_{0:T},\theta,y_{1:T})$ provided $j$ is large enough \cite[Chapter 7][]{Robe:Case:mont:2004}. Initial values $\theta^{(0)}$ and $x_{0:T}^{(0)}$ could be chosen arbitrarily or by sampling from the prior $\prod_{t=1}^T\left\{p(x_t|x_{t-1},\theta)\right\}p(x_0,\theta)$. In either case, the dependence of the resulting chain could be sensitive to the initial values, and generating multiple chains from different starting points could help determine reasonable starting values or burn-in period necessary for the algorithm to generate samples from the posterior distribution \citep{giv:hoet:2005:comp}.
