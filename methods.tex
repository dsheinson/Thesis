...
The general RM algorithm proceeds as follows. Given a particle approximation of the posterior $p(x_{0:t-1},\theta|y_{1:t-1})$ we move to a particle sample approximation of $p(x_{0:t},\theta|y_{1:t})$ by the following steps:
\begin{enumerate}
...
%This is true for the local level DLM described in Section \ref{sec:dlm:ll}, where
%\begin{align}
...
%&\mu_t = \frac{\lambda}{1+\lambda}(y_{t+1} + x_t / \lambda) \nonumber \\
%&\tau^2 = \theta\frac{\lambda}{1+\lambda} \nonumber \\
%&\theta \sim \mbox{IG}(\delta_t,\nu_t) \label{eqn:pl:ll:theta} \\
%&\delta_t = \delta_0 + 1/2 + t \nonumber \\
%&\nu_t = \nu_0 + \frac{1}{2}\left(\sum_{k=1}^t (y_k - x_k)^2 + \frac{1}{\lambda}\sum_{k=1}^t (x_k - x_{k-1})^2 + x_0^2\right) \nonumber
%\end{align}

\subsubsection{PL for dynamic regression}

%as well as for the dynamic regression models described Section \ref{sec:dlm:arwn}, where
%\begin{align}
...
%&\mu_t = \tau^2\left(\frac{(y_{t+1}-U_{t+1}\beta)F_{t+1}}{\sigma^2_m} + \frac{\phi x_t}{\sigma^2_s}\right) \nonumber \\
%&\tau^2 = \left(\frac{F_{t+1}^2}{\sigma^2_m} + \frac{1}{\sigma^2_s}\right)^{-1} \nonumber
%\end{align}
%and sampling from $p(\theta|
