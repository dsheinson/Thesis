...
\caption*{Minimum values of $\phi \in \{0.1,0.2,\ldots,0.9,0.95,0.99\}$ (y-axis) for which log marginal likelihood estimates under $M_{011}$ obtained from three independent PL runs with 500 particles on data simulated from $M_{011}$ with $\beta = (750,15)'$, $\sigma^2_m = 10$, and fixed $\sigma^2_s \in \{0.1,0.2,\ldots,0.5,1.0,1.5,2.0,2.5\}$ (x-axis) are each greater than all three estimates from PL runs on the same data under $M_{101}$ and analytical estimates of the log marginal likelihood under $M_{001}$. Points are displayed for increasing prior standard deviation factor $\kappa$ (plot panels) to determine the prior hyperparameters under $M_{011}$ (see equation \eqref{eqn:fmri:kappa} and description of how inverse-gamma hyperparameters were set in Section \ref{sec:fmri:dist}), and for three separate sets of simulations from $M_{011}$ (colors), where each set consists of simulated time series under all combinations of the aforementioned fixed parameter values. Prior hyperparameters under $M_{101}$ and $M_{001}$ were set as described in Section \ref{sec:fmri:dist} with $\kappa = 1$. If there exists no value of $\phi$ for which $M_{011}$ is distinguished from $M_{101}$ and $M_{001}$ for fixed $\sigma^2_s / \sigma^2_m$ within a given set of simulations, no point is plotted for that value of $\sigma^2_s / \sigma^2_m$.}
\end{figure}
...
\caption*{Minimum values of $\phi \in \{0.1,0.2,\ldots,0.9,0.95,0.99\}$ (y-axis) for which log marginal likelihood estimates under $M_{101}$ obtained from three independent PL runs with 500 particles on data simulated from $M_{101}$ with $\beta = (750,15)'$, $\sigma^2_m = 10$, and fixed $\sigma^2_s \in \{0.1,0.2,\ldots,0.5,1.0,1.5,2.0,2.5\}$ (x-axis) are each greater than all three estimates from PL runs on the same data under $M_{011}$ and analytical estimates of the log marginal likelihood under $M_{001}$. Points are displayed for increasing prior standard deviation factor $\kappa$ (plot panels) to determine the prior hyperparameters under $M_{101}$ (see equation \eqref{eqn:fmri:kappa} and description of how inverse-gamma hyperparameters were set in Section \ref{sec:fmri:dist}), and for three separate sets of simulations from $M_{101}$ (colors), where each set consists of simulated time series under all combinations of the aforementioned fixed parameter values. Prior hyperparameters under $M_{011}$ and $M_{001}$ were set as described in Section \ref{sec:fmri:dist} with $\kappa = 1$. If there exists no value of $\phi$ for which $M_{101}$ is distinguished from $M_{011}$ and $M_{001}$ for fixed $\sigma^2_s / \sigma^2_m$ within a given set of simulations, no point is plotted for that value of $\sigma^2_s / \sigma^2_m$.}
\end{figure}
