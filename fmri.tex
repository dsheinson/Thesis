...
Given the exact marginal likelihood of the data under $M_{001}$, approximations to the marginal likelihood under $M_{101}$ and $M_{011}$ using PL, and prior probability of each model, relative posterior model probabilities among the three models can be computed according to equation \eqref{eqn:modelcomp}. In order to run the PL algorithm and compare the three models in this way, we need to specify 1) the prior distribution of the initial state and fixed parameters, 2) the prior probability of each model, and 3) the number of particles to use in the PL algorithm.
...
The hyperparameters $\vartheta_0$, $B_0$, $\varphi_0$, $\Phi_0$, $a_{m_0}$, $b_{m_0}$, $a_{s_0}$, and $b_{s_0}$ are known, and we let $B_0$ and $\Phi$ have the form
\begin{equation}
B_0 = \kappa^2 \left(\begin{array}{cc} 1000 & 0 \\ 0 & 225 \end{array}\right) \quad \Phi_0 = \kappa^2\times0.25. \label{eqn:fmri:kappa}
\end{equation}
In Sections \ref{sec:fmri:sim} and \ref{sec:fmri:dist}, we analyze simulated data and let $b_0$ and $\phi_0$ be equal to the true values of $\beta$ and $\phi$ used for simulation, respectively, and $a_{m_0}$, $b_{m_0}$, $a_{s_0}$, and $b_{s_0}$ are set such that mean of each inverse-gamma prior is equal to the true value of the corresponding parameter and the variance of both inverse-gamma priors are equal to $\kappa^2\times500$.
