...
\subsection{The correlation approach using the GLM \label{sec:fmri:glm}}
...
The data set that we analyze in Sections \ref{sec:fmri:dr} and \ref{sec:fmri:pl} consists of time series from 5 by 5 by 5 voxel cubes (125 voxels per cube) extracted from six different brain regions from one human subject. The brain regions from which data were extracted are the left frontal pole (FP), left intraparietal sulcus (IPS-left), right intraparietal sulcus (IPS-right), primary visual cortex (PV), left secondary visual cortex (SV-left), and right secondary visual cortex (SV-right). The subject worked on a episodic word recognition task, which consists of an encoding session that takes place outside of the scanner and a recognition session that takes place in the scanner. During encoding, the subject was presented with a list of words presented one at a time and instructed to memorize the words so that if they saw the word again, they would recognize it. During the recognition session, the subject was presented with a another list of words, some of which they would have saw during encoding and some of which were new. The subject was asked to respond as to whether they thought each word was old or new based on their memory. The words were presented according to a rapid-event related design and the onsets were recorded.

For this experiment, the expected BOLD response ($\mbox{conv}_t$) is constructed by convolving the canonical hrf (bottom panel of Figure \ref{fig:fmri:data}) with a boxcar function that is equal to 1 during TRs when a word is presented to the subject and 0 otherwise. The top and middle panels of Figure \ref{fig:fmri:data} show the observed fMRI time series for a voxel in IPS-left and the expected BOLD response for an active voxel, respectively. Notice that the pattern of the observed time series somewhat mirrors an active response for TRs greater than 75, but not for TRs less than 75. This type of behavior motivates our thinking that a regression model with a changing slope, such as $M_{011}$, might be appropriate for modeling fMRI data.
