...
\begin{equation}
v \sim \mbox{N}(0,\sigma^2_m I_T + \lambda Q) \quad Q_{i,j} = \left\{\begin{array}{ll} 0, & \mbox{if } i=j \\ e^{-|i-j|}, & \mbox{if } i\ne j, \end{array} \right.
\end{equation}
...
\begin{equation}
t^* = \frac{\hat{\beta_1}}{\left((X'X)^{-1}X'(\hat{\sigma}^2_m I_T + \hat{\lambda} Q)X(X'X)^{-1}\right)_{(2,2)}}, \label{eqn:fmri:hyp-reml}
\end{equation}
where $\hat{\beta_1}$, $\hat{\sigma}^2_m$, and $\hat{\lambda}$ are the REML estimates. Under the null hypothesis, $t^* \sim \mbox{T}(0,1,df)$, where the degrees of freedom $df$ is computed by the Satterthwaite approximation \citep{wors:frist:color:1995}. In Section \ref{sec:fmri:fpr}, we compare ordinary least squares (OLS) estimation, prewhitening, REML in terms of false positive and true positive rates of significant brain activation using simulated data.
...
\right),
\]
and $W = \sigma^2ee'$ with $e = (1, \gamma_1, \ldots, \gamma_m)'$. The unknown fixed parameters are given by $\theta = (\beta', \phi', \gamma', \sigma^2_s)'$, where $\beta = (\beta_0,\beta_1)'$, $\phi = (\phi_1,\phi_2,\ldots,\phi_P)'$ and $\gamma = (\gamma_1,\gamma_2,\ldots,\gamma_Q)'$. We let $U_t = (1,\mbox{conv}_t)$ and adopt the convention that $\phi_s = 0$ for $s > P$ and $\gamma_r = 0$ for $r > Q$.
...
\end{equation}
where $\hat{\phi}$ maximizes $\log p(y|\phi)$. $X^*(\hat{\phi})$ and $y^*(\hat{\phi})$ are calculated using equations \eqref{eqn:pw:cor} and \eqref{eqn:pw:trans}. The t-test in equation \eqref{eqn:fmri:ttest} is then carried out using these estimates.
...
In this section, we show via a simulated example why examining the independence of residuals from model fits can be misleading in terms of evaluating the ability of different models to sufficiently capture the autocorrelation present in fMRI data.
